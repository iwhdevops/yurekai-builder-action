...
const isAgency = (baseUrl: string): boolean => {
  ...
  return (
    /estates$/.test(url.hostname) || 
    /builder\\/projects$/.test(url.hostname) ||
    /estates/.test(url.hostname) && params.has('uuids') ||
    /builder\\/projects/.test(url.hostname) && params.has('ids')
  )
};
...
const isCockpit = (baseUrl: string): boolean => {
  ...
  return (
    ...
    url.hostname === 'cockpit.yurekai.com'
  );
}
      ...
      response
        ...
        .map( async ({ settings: { baseUrl, commonHeaders: headers = {} } }: any) => {
          ...
          if (isCockpit(baseUrl)) {
            ...
            return response.data?.items.map(({ idProject }: any) => idProject) || [];
          } else {
            return [];
          }
        }),
